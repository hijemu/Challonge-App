...
const ConnectChallonge: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (params.get('connected') === '1') {
      setMessage('Challonge connected successfully.');
      loadStatus();
    }

    if (params.get('connected') === '0') {
      setMessage(params.get('error') || 'Challonge connection failed.');
      loadStatus();
    }
      ...
      <IonContent fullscreen>
        <main className="bbx-page">
          ...
          <div className="bbx-topbar">
            <div>
              ...
              <p className="bbx-subtitle">
                Every organizer connects their own account. Your app stays clean and multi-user.
              </p>
            </div>
          ...
          <section className="bbx-status-card">
            ...
            <h2 className="bbx-title" style={{ fontSize: 32 }}>
              Run brackets with your own account.
            </h2>
            <p className="bbx-subtitle">
              OAuth means no sharing API keys. Staff log in, connect Challonge, then control their tournaments.
            </p>
          ...
          <div className="bbx-connect-panel" style={{ marginTop: 16 }}>
            <button className="bbx-button primary" onClick={connectOAuth}>
              {connected ? 'Reconnect Challonge' : 'Connect with Challonge'}
            </button>
            <button className="bbx-button ghost" onClick={() => history.push('/tournaments')}>
              Open My Tournaments
            </button>
            {connected && (
              <button className="bbx-button danger" onClick={disconnect}>
                Disconnect
              </button>
            )}
          </div>
          ...
          <section className="bbx-soft-card" style={{ padding: 18 }}>
            <p className="bbx-subtitle" style={{ marginTop: 0 }}>
              Use this only while waiting for Challonge Connect OAuth keys.
            </p>
            ...
            <button
              ...
              onClick={connectDevKey}
            >
              Connect API Key for Dev
            </button>
          </section>
        </main>
      </IonContent>
